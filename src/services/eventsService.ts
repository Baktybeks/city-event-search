...
export const eventsApi = {
  ...
  getEvents: async (filters?: EventFilters, limit = 20, offset = 0) => {
    try {
      ...
      if (isNetworkError(error)) {
        console.warn(
          "⚠️ getEvents: Сеть недоступна. Проверьте Appwrite и подключение."
        );
  ...
  getFeaturedEvents: async (limit = 6) => {
    try {
      ...
      if (isNetworkError(error)) {
        console.warn(
          "⚠️ getFeaturedEvents: Сеть недоступна. Проверьте Appwrite и подключение."
        );
        return [];
      }
  ...
  getEventsByCategory: async (category: EventCategory, limit = 10) => {
    try {
      ...
      if (isNetworkError(error)) {
        console.warn(
          "⚠️ getEventsByCategory: Сеть недоступна. Проверьте Appwrite и подключение."
        );
        return [];
      }
  ...
  getEventById: async (
    ...
    try {
      ...
      if (isNetworkError(error)) {
        console.warn(
          "⚠️ getEventById: Сеть недоступна. Проверьте Appwrite и подключение."
        );
        throw error;
      }
  ...
  getOrganizerEvents: async (organizerId: string) => {
    try {
      ...
      if (isNetworkError(error)) {
        console.warn(
          "⚠️ getOrganizerEvents: Сеть недоступна. Проверьте Appwrite и подключение."
        );
        return [];
      }
  ...
  getUserFavorites: async (userId: string) => {
    try {
      ...
      if (isNetworkError(error)) {
        console.warn(
          "⚠️ getUserFavorites: Сеть недоступна. Проверьте Appwrite и подключение."
        );
        return [];
      }
  ...
  getAllEventsForAdmin: async (
    ...
    try {
      ...
      if (isNetworkError(error)) {
        console.warn(
          "⚠️ getAllEventsForAdmin: Сеть недоступна. Проверьте Appwrite и подключение."
        );
        return { events: [], total: 0 };
      }
